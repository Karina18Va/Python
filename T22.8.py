...
def contents_of_the_folder(folder,
                           filename="output_22_08_" + datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".txt"):
    ...
    for root, dirs, files in os.walk(folder):
        for dir in dirs:
            print(root, " dir ", dir, "",
                  time.strftime("%Y.%m.%d %H:%M:%S", time.localtime(path.getmtime(path.join(root, dir)))))
        for file in files:
            print(root, " file ", file, "",
                  time.strftime("%Y.%m.%d %H:%M:%S", time.localtime(path.getmtime(path.join(root, file)))))
    sys.stdout = oldout
    newout.close()
...
def compare(file1, file2):
    ...
    f1 = open(file1, "r")
    f2 = open(file2, "r")
    if datetime.datetime(*[int(el) for el in re.split("_", file2[-23:-4])]) < datetime.datetime(
            *[int(el) for el in re.split("_", file1[-23:-4])]):
        f1, f2 = f2, f1
    c1 = f1.readlines()
    c2 = f2.readlines()
    ...
    if c1 == c2:
        ...
        print(
            f"Зміни, які відбулися в {c1[0].split('  ')[0]} з {c1[0].split('  ')[1][:-1]} по {c2[0].split('  ')[1][:-1]}:")
        for el in fs1:
            if el not in fs2:
                if el[1] == "dir":
...
if __name__ == "main":
    compare("output_22_08_2022_11_08_04_03_16.txt", "output_22_08_2022_11_08_04_04_09.txt")